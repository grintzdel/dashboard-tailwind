import React, { JSX } from 'react'
import { Avatar } from '@/ui/avatar'
import { Input } from '@/ui/input'
import { IconType } from 'react-icons'

type UserInfoProps = {
  avatarSrc?: string
  name: string
  initials?: string
}

const UserInfo: React.FC<UserInfoProps> = ({ avatarSrc, name, initials }: UserInfoProps): JSX.Element => {
  const today = new Date().toLocaleDateString('en-US', {
    weekday: 'long',
    year: 'numeric',
    month: 'long',
    day: 'numeric',
  })

  return (
    <div className="flex flex-row items-center gap-5">
      <Avatar src={avatarSrc} alt={`${name} Avatar`} initials={initials} className="h-13 w-13" />
      <div className="flex flex-col gap-1">
        <h2 className="text-lg font-semibold text-gray-800">Hey, {name}</h2>
        <p className="text-sm text-gray-500">{today}</p>
      </div>
    </div>
  )
}

type SearchSectionProps = {
  placeholder?: string
}

const SearchSection: React.FC<SearchSectionProps> = ({ placeholder }: SearchSectionProps): JSX.Element => {
  return (
    <div className="w-80">
      <Input placeholder={placeholder} />
    </div>
  )
}

type ActionButtonProps = {
  icon: IconType
  onClick?: () => void
}

const ActionButton: React.FC<ActionButtonProps> = ({ icon: Icon, onClick }: ActionButtonProps): JSX.Element => {
  return (
    <button
      onClick={onClick}
      className="flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-md transition-colors hover:bg-gray-50"
    >
      <Icon className="h-5 w-5 text-gray-600" />
    </button>
  )
}

type HeaderAction = {
  icon: IconType
  onClick?: () => void
}

type HeaderActionsProps = {
  searchPlaceholder?: string
  actions: HeaderAction[]
}

const HeaderActions: React.FC<HeaderActionsProps> = ({
  searchPlaceholder,
  actions,
}: HeaderActionsProps): JSX.Element => {
  return (
    <div className="flex flex-row items-center gap-5">
      <SearchSection placeholder={searchPlaceholder} />
      {actions.map((action, index) => (
        <ActionButton key={index} icon={action.icon} onClick={action.onClick} />
      ))}
    </div>
  )
}

export type HeaderProps = {
  user: UserInfoProps
  searchPlaceholder?: string
  actions: HeaderAction[]
}

export const Header: React.FC<HeaderProps> = ({ user, searchPlaceholder, actions }: HeaderProps): JSX.Element => {
  return (
    <header className="fixed top-0 right-0 left-64 z-10 h-20 bg-[#f5f6f8] px-8">
      <div className="flex h-full flex-row items-center justify-between">
        <UserInfo {...user} />
        <HeaderActions searchPlaceholder={searchPlaceholder} actions={actions} />
      </div>
    </header>
  )
}
